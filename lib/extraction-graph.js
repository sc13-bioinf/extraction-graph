...
return function ()
{
	...
	my["node_modules"]["json-in-neo4j"] = require ("json-in-neo4j");
	...
	my["load_pages"] = function (pages)
	{
		return function (root_node, _)
		{
			...
			for ( var pageId in Object.keys (pages) )
			{
				...
				var page_task = (function (local_pageId, local_pageJSON)
				{
					return function (callback)
					{
						...
						console.log ("start json in neo4j for page "+local_pageId);
						var json_in_neo4j = my["node_modules"]["json-in-neo4j"] (local_pageJSON, "http://localhost:7474");

						json_in_neo4j ();
						console.log ("stop json in neo4j for page "+local_pageId);
